...
/// Kaia 프로바이더 클래스
class KaiaProvider: KaiaProviderInterface {
    ...
    func setNetworkChangeCallback(_ callback: @escaping (String, String) -> Void) {
        ...
        webViewMethodHandler.setNetworkChangeCallback { [weak self] currentNet, newNet in
            print("Kaia WebView network change: \(currentNet) -> \(newNet)")
    ...
    func request(method: String, params: [Any]?) async throws -> Any {
        switch method {
        case "eth_requestAccounts", "eth_accounts", "klay_accounts", "klay_requestAccounts", "kaia_accounts", "kaia_requestAccounts":
            return try await requestAccounts()
            
        case "eth_sendTransaction", "klay_sendTransaction", "kaia_sendTransaction":
        ...
        case "eth_signTransaction", "klay_signTransaction", "kaia_signTransaction":
            guard let params = params, let firstParam = params.first as? [String: Any] else {
                throw EVMError.invalidParams
            }
            return try await signTransaction(transaction: firstParam)
            
        case "eth_signTypedData_v1", "klay_signTypedData_v1":
        ...
        case "eth_signTypedData_v3", "eth_signTypedData_v4", "klay_signTypedData_v3", "klay_signTypedData_v4":
            guard let params = params, params.count >= 2,
                  let address = params[0] as? String,
